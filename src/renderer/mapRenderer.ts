...
export class MapRenderer {
  ...
  async renderLevel(level: Level, generateThumbnail = true, generateScreenshot = true): Promise<void> {
    try {
  ...
  private async renderMap(ctx: CanvasRenderingContext2D, mapData: MapData, size: { width: number; height: number }): Promise<void> {
    try {
  ...
  async renderAllLevels(outputDir: string, progressCallback?: (current: number, total: number, levelName: string) => void): Promise<void> {
    try {
