...
export class DiscordIndexer {
  ...
  async indexDiscord(
    ...
    try {
      ...
      logger.success(
        `Discord indexing completed: ${levelsProcessed} levels processed, ${levelsSkipped} skipped`
      );
  ...
  private async setupDiscordSession(): Promise<void> {
    try {
      ...
      const isLoggedIn = await this.page
        .waitForSelector('[data-list-id="guildsnav"]', {
          timeout: 5000,
        })
        .catch(() => null);
      ...
      if (email && password) {
        ...
        console.log(
          chalk.white('This window will wait for up to 5 minutes for login to complete.')
        );
        console.log(chalk.yellow('===============================\n'));
      }
  ...
  private async scrapeForumChannel(channelUrl: string): Promise<DiscordMessage[]> {
    ...
    while (!foundCrystalClicker && consecutiveEmptyChunks < 3) {
      ...
      if (visiblePosts.length > 0) {
        ...
        logger.info(
          `Visible chunk: "${firstTitle}" ... "${lastTitle}" (${visiblePosts.length} posts)`
        );
      }

      // Find overlap with previous chunk
      let overlapIndex = -1;
      const currentChunkIds = visiblePosts.map(p => p.id);
      ...
      for (let i = 0; i < visiblePosts.length; i++) {
        ...
        const alreadyHasMessages = allMessages.some(msg => msg.id && msg.id.includes(post.id));
      ...
      logger.info(
        `Chunk complete: processed ${processedInThisChunk} new posts, total unique: ${totalProcessed}`
      );
      ...
      if (!foundCrystalClicker && !isStuck) {
        ...
        if (newPostsInChunk || totalProcessed === 0) {
          if (possiblyAtEnd && !foundCrystalClicker) {
            logger.info(
              'Possibly at end of list (fewer posts visible). Scrolling more to check for remaining posts...'
            );
            ...
            const container = await this.page.$(
              'div[data-list-id="forum-channel-list-1139908458968252457"]'
            );
            if (container) {
              await container.focus();
            ...
            logger.info(
              `Target post for next chunk: "${targetPost?.title || 'unknown'}" should become one of the first posts`
            );
            ...
            logger.info(
              `Scrolling down by ${scrollSteps} arrow presses (${baseScrollSteps} base + ${extraScroll} extra)`
            );
            ...
            const containerElement = await this.page.$(
              'div[data-list-id="forum-channel-list-1139908458968252457"]'
            );
            if (containerElement) {
            ...
            logger.info(
              `After scroll: Target post "${targetPost?.title}" is at index ${targetIndex} of ${afterScroll.length} visible posts`
            );
            ...
            if (foundTarget && targetIndex > 5) {
              ...
              logger.warn(
                `Target post at index ${targetIndex}, need ${additionalScrolls} more scrolls`
              );
              ...
              if (firstPost) {
                logger.info(
                  `Target post not found. First visible post is now: "${firstPost.title}"`
                );

                // If we see the same first post as before, the list might have reset
                if (visiblePosts[0] && firstPost.id === visiblePosts[0].id) {
    ...
    logger.info(
      `Chunk processing complete. Found ${totalProcessed} unique forum posts out of ~90 total`
    );
    logger.info(
      `Found Rage Road: ${foundRageRoad} | Found Crystal Clicker: ${foundCrystalClicker}`
    );

    if (!foundCrystalClicker && totalProcessed > 50) {
      logger.warn(
        "Processed over 50 posts but haven't found Crystal Clicker yet. It should be near the end of ~90 posts."
      );
    }
  ...
  private async processForumPost(post: ForumPost): Promise<DiscordMessage[]> {
    ...
    try {
        ...
        messageElements.forEach(element => {
          ...
          links.forEach(link => {
            ...
            if (
              ...
              (text.toLowerCase().includes('.dat') || href.toLowerCase().includes('.dat'))
            ) {
      ...
      if (extractedData.error) {
        ...
        logger.info(
          `Found ${extractedData.messages.length} messages with .dat files in post: ${post.title}`
        );
