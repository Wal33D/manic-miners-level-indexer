...
import { DiscordBaseIndexer, DiscordThread, DiscordAPIMessage } from './discordBase';
import fetch from 'node-fetch';

export class DiscordCommunityIndexer extends DiscordBaseIndexer {
  constructor(
    channels: string[],
    outputDir: string,
  ...
  async indexDiscord(
    ...
    try {
      ...
      for (let channelIndex = 0; channelIndex < this.channels.length; channelIndex++) {
        ...
        const channelMessages = await this.fetchForumChannelMessages(channelId);
      ...
      logger.success(
        `Discord Community indexing completed: ${levelsProcessed} levels processed, ${levelsSkipped} skipped`
      );
  ...
  private async fetchForumChannelMessages(channelId: string): Promise<DiscordMessage[]> {
    ...
    try {
      ...
      if (channelInfo.type !== 15) {
        logger.warn(`Channel ${channelId} is not a forum channel (type: ${channelInfo.type})`);
        return messages;
      }
      ...
      const archivedThreads = await this.fetchArchivedThreads(channelId);

      // Fetch active threads
      const activeThreads = await this.fetchActiveThreads(channelId);

      const allThreads = [...archivedThreads, ...activeThreads];
      logger.info(`Total threads to process: ${allThreads.length}`);
      ...
      logger.error(`Failed to fetch messages from forum channel ${channelId}:`, error);
      throw error;
    }
  ...
  private async fetchArchivedThreads(channelId: string): Promise<DiscordThread[]> {
    ...
    while (hasMore) {
      ...
      try {
        ...
        const batchThreads: DiscordThread[] = archivedData.threads || [];

        logger.info(`Found ${batchThreads.length} archived threads in this batch`);
        threads.push(...batchThreads);

        // Check if there are more threads
        hasMore = archivedData.has_more;

        if (hasMore && batchThreads.length > 0) {
          // Get the last thread's archive timestamp for pagination
          const lastThread = batchThreads[batchThreads.length - 1];
    ...
    logger.info(`Total archived threads found: ${threads.length}`);
    return threads;
  }

  private async fetchActiveThreads(channelId: string): Promise<DiscordThread[]> {
    const threads: DiscordThread[] = [];
