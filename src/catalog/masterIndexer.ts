...
export class MasterIndexer {
  ...
  constructor(config: IndexerConfig) {
    ...
    if (config.sources.archive.enabled) {
      this.archiveIndexer = new ArchiveIndexer(
        config.sources.archive.baseUrl,
        999, // Unlimited - fetch all available items
        config.outputDir,
        config.sources.archive.concurrentDownloads || 5
      );
    }
  ...
  async indexAll(): Promise<void> {
    try {
      ...
      if (this.archiveIndexer) {
        logger.info('Starting Internet Archive indexing...');
        const result = await this.archiveIndexer.indexArchive((progress) => {
      ...
      logger.success(`Master indexing completed: ${totalProcessed} levels processed, ${totalErrors} errors`);
    ...
    try {
      ...
      switch (source) {
        case MapSource.ARCHIVE:
          if (this.archiveIndexer) {
            await this.archiveIndexer.indexArchive((progress) => {
              logger.progress(progress.message, progress.current, progress.total);
            });
          }
          break;
  ...
  private async getSourceSummary(): Promise<Record<MapSource, { count: number; lastUpdated: string }>> {
    const allLevels = await this.getAllLevels();
  ...
  async getCatalogStats(): Promise<{ totalLevels: number; sources: Record<MapSource, number>; lastUpdated: Date }> {
    await this.catalogManager.loadCatalogIndex();
